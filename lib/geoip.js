...
export function extractClientIps(req) {
  ...
  if (cfIp) candidates.push(stripIpv6Mapped(String(cfIp).trim()));

  const remote = req.socket?.remoteAddress;
  const trustForwarded = !!cfIp || isTrustedProxy(remote);
  ...
  if (trustForwarded) {
    ...
    if (realIp) candidates.push(stripIpv6Mapped(String(realIp).trim()));
  }

  if (remote) candidates.push(stripIpv6Mapped(remote));
